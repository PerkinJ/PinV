...
// 从数据集中返回最大的 X 坐标
var xMax = function xMax(data, key) {
...
// 从数据集返回最大的 Y 坐标
var yMax = function yMax(data, key) {
	return d3.max(data, function (d) {
		return d[key];
	});
};
...
var xScale = function xScale(props) {
	...
	return d3.scaleLinear().domain([0, xMax(data, XAxis)]).range([padding.left - 1, width - padding.left - 1]); // 这里主要解决坐标轴的偏差
};
...
var yScale = function yScale(props) {
	...
	return d3.scaleLinear().domain([0, yMax(data, YAxis)]).range([height - padding.top - padding.bottom, padding.top]);
};
var ScatterPlot = (_temp = _class = function (_Component) {
	...
	function ScatterPlot(props) {
		...
		_this2.renderData = function () {
			...
			var scatterPlot = d3.select(_this2.scatterPlot);
			var rect = scatterPlot.selectAll('rect');
	...
	ScatterPlot.prototype.render = function render(props, _ref) {
		var _this3 = this;
		...
		return (0, _preact.h)(
			...
			(0, _preact.h)(
				...
				(0, _preact.h)(_Axis2.default, {
					...
					'class': _index2.default.axis,
					transform: 'translate(' + padding.left + ',' + padding.top + ')' }),
				(0, _preact.h)(_Circles2.default, _extends({}, props, scales)),
